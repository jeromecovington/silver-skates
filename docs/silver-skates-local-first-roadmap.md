...
- `yarn ingest [--days 1]`  
- `yarn embed`  
- `yarn cluster [--algo kmeans|hdbscan] [--k 10]`  
- `yarn summarize [--level article|cluster|daily]`  
- `yarn evaluate`  
...
### Appendix A — Example Interfaces (TypeScript)

```ts
// Model provider abstraction
export interface ModelProvider { 
  name(): string;
  tokensPerSecond(): Promise<number | null>;
  generate(prompt: string, opts?: { maxTokens?: number; temperature?: number; seed?: number }): Promise<string>;
  embed(texts: string[], opts?: { batchSize?: number }): Promise<number[][]>;
}

// KMeans runner
export type ClusterRunConfig = { k: number; seed: number; embedModel: string; algo: 'kmeans'|'hdbscan' };
```
